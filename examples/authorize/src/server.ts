...
const localnetPayerAccount = mnemonicToAccount(localnetMnemonic, { accountIndex: 2 })
...
const account =
  network === 'localnet' && !process.env.MPPX_SERVER_PRIVATE_KEY
    ? mnemonicToAccount(localnetMnemonic, { accountIndex: 0 })
    : privateKeyToAccount((process.env.MPPX_SERVER_PRIVATE_KEY as Hex) ?? generatePrivateKey())
...
if (network === 'localnet') await setupLocalnet()
else
...
async function setupLocalnet() {
  ...
  for (const id of [1n, 2n, 3n]) {
    await Actions.amm.mintSync(client, {
      ...
      validatorTokenAmount: parseUnits('1000', decimals),
    })
  }

  for (const token of [currency, Addresses.pathUsd as Address]) {
    await Actions.token.transferSync(client, {
      account,
      amount: parseUnits('10000', decimals),
      token,
      to: localnetPayerAccount.address,
    })
  }
}
