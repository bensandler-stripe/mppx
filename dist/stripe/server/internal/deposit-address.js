 ...
 */
export async function resolveDepositAddress(secretKey, network) {
    const headers = {
    ...
    try {
        const listResponse = await fetch(`https://api.stripe.com/v1/crypto/deposit_addresses?network=${network}&limit=1`, { headers });
        if (listResponse.ok) {
