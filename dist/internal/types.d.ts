...
export type DefinedKeys<type> = {
    [key in keyof type]-?: {} extends Pick<type, key> ? never : undefined extends type[key] ? never : key;
}[keyof type];
/**
 ...
 * @internal
 */
export type ConfiguredDefaults<parameters, defaults> = Pick<parameters, Extract<DefinedKeys<parameters>, keyof defaults>>;
/** @internal */
 ...
 * @internal
 */
export type UnionMerge<A, B> = [keyof B] extends [never] ? A : LooseOmit<A, keyof B & string> & {
 ...
 * @internal
 */
export type NoExtraKeys<T, Shape> = [T] extends [Shape] ? T & {
    [K in Exclude<keyof T, KeysOfUnion<Shape>>]: never;
} : never;
/** @internal */
