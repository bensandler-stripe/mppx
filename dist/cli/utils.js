...
export function printRequestHeaders(reqUrl, init, info) {
...
export function printResponseHeaders(res, opts) {
    if (!opts.include && opts.verbose < 2)
        return;
    if (opts.silent)
        return;
...
export function fmtRequestValue(key, value, ctx) {
...
export function fmtChallengeValue(key, value) {
...
export function parseMethodOpts(raw) {
    if (!raw)
        return {};
...
export function isAgentEnvironment(env = process.env) {
    ...
    for (const [key, value] of Object.entries(env)) {
        if (!value)
            continue;
        if (key === 'CODEX' || key.startsWith('CODEX_'))
            return true;
    }
    return false;
}
export function isTempoAccount(accountName) {
...
export function confirm(prompt, defaultYes = false) {
    const reader = readline.createInterface({ input: process.stdin, output: process.stderr });
    return new Promise((resolve) => {
...
export function fmtBalance(b, symbol, decimals = 6, opts) {
...
export function resolveRpcUrl(explicit, options = {}) {
    return (explicit ??
...
export function resolveFundingNetwork(options = {}) {
    if (options.network)
        return options.network;
    if (options.rpcUrl)
        return undefined;
    if (process.env.MPPX_RPC_URL?.trim() || process.env.RPC_URL?.trim())
        return undefined;
    return 'testnet';
}
export async function resolveChain(opts = {}) {
    ...
    const allExports = Object.values(await import('viem/tempo/chains'));
    ...
    if (!found)
        throw new Error(`Unknown chain ID ${chainId} from RPC ${rpcUrl}`);
    return found;
}
...
export const mainnetTokens = [pathUsd, usdc];
...
export async function fetchTokenInfo(client, token, account) {
    ...
    const decimals = 'decimals' in metadata && typeof metadata.decimals === 'number' ? metadata.decimals : 6;
    return { balance, symbol, decimals, token };
}
export async function fetchBalanceLines(address, opts) {
    if (opts?.chain) {
