...
import { orderTempoChargeChallengesByBalance, readTempoKeystore, resolveTempoAccount, } from './plugins/tempo.js';
...
import { chainName, confirm, decodeMemo, fetchBalanceLines, fmtBalance, fmtChallengeValue, fmtRequestValue, isTempoAccount, link, parseMethodOpts, pc, printRequestHeaders, printResponseHeaders, prompt, resolveChain, resolveFundingNetwork, resolveRpcUrl, } from './utils.js';
...
async function writeResponseBody(response) {
...
function filterChallengesByCurrency(challenges, currency) {
    if (!currency)
        return [...challenges];
...
const cli = Cli.create('mppx', {
    ...
    options: z.object({
        ...
        session: z
            ...
            .optional()
            .default('auto')
            .refine((value) => value === 'auto' || value === 'new' || /^0x[0-9a-fA-F]{64}$/.test(value), 'Expected auto, new, or a 32-byte channel ID')
    ...
    async run(c) {
        ...
        try {
            ...
            {
                printResponseHeaders(challengeResponse, challenge.method === 'tempo' && challenge.intent === 'session' && c.options.include
            ...
            const persistentSessionAccount = process.env.MPPX_PRIVATE_KEY?.trim() ||
            ...
            if (pluginResult?.createCredential)
                ...
                credential = await mppx.createCredential(selectedChallengeResponse, pluginResult.credentialContext);
            }
...
const account = Cli.create('account', {
    ...
    async run(c) {
        ...
        return outputResult(c, { address: acct.address, name: resolvedName }, () => {
            ...
            console.log(pc.dim(`Fund testnet tokens: mppx account fund --account ${resolvedName} --network testnet`));
        });
    },
})
    .command('default', {
