...
import { createSessionRegistry, SessionBusyError, SessionStateError, sessionResourceUrl, sessionScope, toChannelStore, } from './store.js';
...
export function sessionCommandError(error, fallbackCode) {
...
async function resolveCloseAccount(record, accountOverride) {
...
async function resolveCloseClient(chainId, options) {
...
const sessions = Cli.create('sessions', {
    ...
    async run(c) {
        try {
            const records = (await createSessionRegistry().list())
                .filter((record) => {
                if (c.options.account && record.account.name !== c.options.account)
                    return false;
    ...
    async run(c) {
        ...
        async function closeSession(channelId, options = {}) {
            ...
            try {
                ...
                const state = await Chain.getChannelState(client, record.channel.channelId, record.channel.escrow);
                if (state.deposit === 0n) {
                ...
                const closingContext = (challenge = record.challenge) => ({
                    status: 'closing',
                    account,
                    endpoint,
                    challenge,
                ...
                const closeFetch = async (input, init) => globalThis.fetch(input, {
                    ...init,
        ...
        try {
            if (c.options.all) {
                const records = (await registry.list()).filter((record) => (!c.options.account || record.account.name === c.options.account) &&
                    (!c.options.network || record.channel.chainId === networkChainId(c.options.network)));
                const closed = [];
                const failed = [];
                for (const record of records) {
                    try {
                        closed.push(await closeSession(record.channel.channelId, {
                ...
                for (const item of closed)
                    console.log(`${item.channelId}  ${item.status}  ${item.spent}`);
                for (const failure of failed)
