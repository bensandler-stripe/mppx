...
import { canSignDescriptor, resolveChallengeContext, } from '../../tempo/session/client/CredentialState.js';
...
import { isTestnet, printResponseHeaders, resolveChain, resolveRpcUrl, } from '../utils.js';
import { createSessionRegistry, SessionBusyError, sessionResourceUrl, sessionScope, sessionScopeKey, toChannelStore, } from './store.js';
/** Resolves `--session` and the `-M channel=` compatibility alias. */
export function resolveSessionSelection(session, channelAlias) {
...
function sessionDecimals(challenge) {
...
export function resolveSessionMaxDeposit(challenge, methodOptions, testnet) {
    if (methodOptions.deposit !== undefined)
        return methodOptions.deposit;
...
function validateReceipt(receipt, parameters) {
    if (!receipt)
        return undefined;
    ...
    if (spent > accepted)
        throw new Error('Session receipt spend exceeds its accepted authorization.');
    return receipt;
}
function sameChannel(record, channelId) {
...
export async function runPersistentSessionRequest(parameters) {
    ...
    const lock = await registry.acquire(scope).catch((cause) => {
    ...
    try {
        const selectedId = selection === 'auto'
        ...
        let status = reusable ? 'open' : 'opening';
        ...
        const manager = sessionManager({
            ...
            fetch: async (input, init) => {
                if (!replayPending)
                    return parameters.fetch(input, init);
                replayPending = false;
        ...
        if (!channelId)
            throw new Error('Session manager did not select a channel.');
        latestReceipt =
        ...
        if (isEventStream(response)) {
            const stream = getSessionManagerInternals(manager).consumeSseResponse(parameters.fetchInput, response, { onReceipt, signal: parameters.init.signal ?? undefined });
        ...
        if (latestReceipt) {
            ...
            if (!validatedReceipt)
                throw new Error('Session receipt validation failed.');
