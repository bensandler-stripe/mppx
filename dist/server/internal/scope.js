...
export function attach(request, scope) {
...
export function merge(parameters) {
    ...
    if (scope !== undefined && metaScope !== undefined && metaScope !== scope) {
        throw new Error(`Conflicting scope values: \`scope\` (${scope}) does not match \`meta.${reservedMetaKey}\` (${metaScope}).`);
    }
    if (scope === undefined || metaScope === scope)
        return meta;
