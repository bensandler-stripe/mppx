...
function getBalanceCall(client, parameters) {
    const call = Actions.token.getBalance.call;
    return call.length >= 2 ? call(client, parameters) : call(parameters);
}
// TODO: Remove once the minimum viem version is >=2.54.0, which uses client-first Tempo call builders.
function getApproveCall(client, parameters) {
    const call = Actions.token.approve.call;
    return call.length >= 2 ? call(client, parameters) : call(parameters);
}
// TODO: Remove once the minimum viem version is >=2.54.0, which uses client-first Tempo call builders.
function getBuyCall(client, parameters) {
    const call = Actions.dex.buy.call;
    return call.length >= 2 ? call(client, parameters) : call(parameters);
}
/**
...
export async function findCalls(client, parameters) {
    ...
    const balanceResults = await Promise.allSettled([
        readContract(client, getBalanceCall(client, { account, token: tokenOut })),
        ...candidates.map((t) => readContract(client, getBalanceCall(client, { account, token: t }))),
    ]);
