...
export function sessionManager(parameters) {
    ...
    const WebSocketImpl = parameters.webSocket ??
        globalThis.WebSocket;
    const maxVoucherCumulative = parameters.maxDeposit !== undefined
    ...
    let spent = 0n;
    let activeSocketChallenge = null;
    let activeSocketChannelId = null;
    let activeSocket = null;
    let closeReadyReceipt = null;
    let closeReadyWaiter = null;
    let expectedSocketCloseAmount = null;
    let receiptWaiter = null;
    ...
    function assertReceiptWithinLocalState(receipt) {
    ...
    function waitForReceipt(predicate = () => true) {
        if (receiptWaiter)
            throw new Error('receipt wait already in progress');
    ...
    function waitForCloseReady() {
        if (closeReadyReceipt)
            return Promise.resolve(closeReadyReceipt);
        if (closeReadyWaiter)
            throw new Error('close-ready wait already in progress');
        return new Promise((resolve, reject) => {
            closeReadyWaiter = { resolve, reject };
        });
    }
    function settleReceipt(receipt) {
        if (!receiptWaiter)
            return;
        if (!receiptWaiter.predicate(receipt))
            return;
        const waiter = receiptWaiter;
        receiptWaiter = null;
        waiter.resolve(receipt);
    }
    function settleCloseReady(receipt) {
    ...
    function rejectCloseReady(error) {
        if (!closeReadyWaiter)
            return;
        const waiter = closeReadyWaiter;
        closeReadyWaiter = null;
    ...
    function createManagedSocket(socket) {
        ...
        const emit = (type, event) => {
            ...
            const handler = managed[property];
    ...
    const self = {
        ...
        async open(options) {
            if (channel?.opened)
                return;
            if (!lastChallenge) {
                throw new Error('No challenge available. Make a request first to receive a 402 challenge, or pass a challenge via .fetch()/.sse().');
            }
            ...
            if (!lastUrl)
                throw new Error('No URL available — call fetch() or sse() before open().');
            ...
            if (!response.ok) {
                ...
                throw new Error(`Open request failed with status ${response.status}${body ? `: ${body}` : ''}${wwwAuth ? ` [${Constants.Headers.wwwAuthenticate}: ${wwwAuth}]` : ''}`);
            }
        },
        fetch: doFetch,
        ...
        async ws(input, init) {
            ...
            const candidates = AcceptPayment.selectChallengeCandidates(Challenge.fromResponseList(probe), [method], AcceptPayment.resolve([method]).entries);
            const challenge = (await resolveSessionChallengeOrder(candidates, init?.orderChallenges ?? parameters.orderChallenges))[0]?.challenge;
            if (!challenge) {
            ...
            wsTickCost = BigInt(challenge.request.amount);
            ...
            const isExpectedReceipt = (receipt) => receipt.challengeId === challenge.id && receipt.channelId === activeSocketChannelId;
            ...
            rawSocket.addEventListener('close', (event) => {
                if (activeSocket === rawSocket)
                    activeSocket = null;
                if (activeSocketChallenge === challenge)
                    activeSocketChallenge = null;
                if (activeSocketChannelId === openCredential.payload.channelId)
                ...
                switch (message.mpp) {
                    ...
                    case 'payment-need-voucher': {
                        ...
                        try {
                            ...
                            failSocketFlow(error instanceof Error
                                ? error.message
                                : 'requested voucher amount exceeds local maxDeposit');
                            break;
                        }
                        const nextCumulative = (channel?.cumulativeAmount ?? 0n) > required
                    ...
                    case 'payment-receipt':
                        ...
                        if (expectedSocketCloseAmount !== null &&
...
async function resolveSessionChallengeOrder(candidates, override) {
