...
function resolveVoucherSigner(account, voucherSigner) {
...
export async function createVoucherPayload(client, account, channelId, cumulativeAmount, escrowContract, chainId, voucherSigner) {
    const signer = resolveVoucherSigner(account, voucherSigner);
    const signature = await signVoucher(client, account, { channelId, cumulativeAmount }, escrowContract, chainId, signer);
    return {
        action: 'voucher',
        channelId,
...
export async function createClosePayload(client, account, channelId, cumulativeAmount, escrowContract, chainId, voucherSigner) {
    const signer = resolveVoucherSigner(account, voucherSigner);
    const signature = await signVoucher(client, account, { channelId, cumulativeAmount }, escrowContract, chainId, signer);
    return {
        action: 'close',
...
export async function createOpenPayload(client, account, options) {
    ...
    const signature = await signVoucher(client, account, { channelId, cumulativeAmount: initialAmount }, escrowContract, chainId, voucherSigner);
    return {
        entry: {
            authorizedSigner,
            channelId,
    ...
    try {
        ...
        if (onChain.deposit > 0n && !onChain.finalized) {
            return {
                ...
                authorizedSigner: BigInt(onChain.authorizedSigner) === 0n ? onChain.payer : onChain.authorizedSigner,
                channelId,
                salt: '0x',
