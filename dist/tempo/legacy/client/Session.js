...
export function session(parameters = {}) {
    ...
    async function autoManageCredential(challenge, account, context) {
        ...
        if (entry?.opened) {
            ...
            payload = await createVoucherPayload(client, account, entry.channelId, entry.cumulativeAmount, escrowContract, chainId, voucherSigner);
            notifyUpdate(entry);
        }
        else {
    ...
    async function manualCredential(challenge, account, context) {
        ...
        switch (action) {
            case 'open': {
                ...
                const signature = await signVoucher(client, account, { channelId, cumulativeAmount }, escrowContract, chainId, voucherSigner);
                payload = {
                    action: 'open',
                    type: 'transaction',
                    channelId,
            ...
            case 'voucher': {
                ...
                payload = await createVoucherPayload(client, account, channelId, cumulativeAmount, escrowContract, chainId, voucherSigner);
    ...
    return Method.toClient(Methods.session, {
        canHandleChallenge({ challenge }) {
            const sessionProtocolMarker = Constants.getMethodDetail(challenge.request.methodDetails, Constants.MethodDetailKeys.sessionProtocol);
            return (sessionProtocolMarker === undefined ||
