...
import { AmountExceedsDepositError, BadRequestError, ChannelClosedError, ChannelNotFoundError, DeltaTooSmallError, InsufficientBalanceError, InvalidSignatureError, VerificationFailedError, } from '../../../Errors.js';
...
import { broadcastOpenTransaction, broadcastTopUpTransaction, closeOnChain, getOnChainChannel, settleOnChain, } from '../session/Chain.js';
...
export function session(p) {
    const parameters = p;
    const { amount, channelStateTtl = 5_000, currency = defaults.resolveCurrency(parameters), decimals = defaults.decimals, feePayerPolicy, store: rawStore = Store.memory(), suggestedDeposit, unitType, waitForConfirmation = true, } = parameters;
    const lastOnChainVerified = new Map();
    const store = ChannelStore.fromStore(Store.from(rawStore, { keyPrefix: parameters.storeKeyPrefix }));
    const { account, recipient, feePayer, feePayerUrl } = Account.resolve(parameters);
    if (!account)
        throw new Error('tempo.session() requires an `account` (viem Account, e.g. privateKeyToAccount("0x...")). An address string is not sufficient — the server needs a signing account for on-chain channel close and settlement.');
    ...
    return Method.toServer(Methods.session, {
        ...
        async request({ credential, request }) {
            ...
            const resolvedEscrow = request.escrowContract ??
        ...
        async verify({ credential, envelope, request }) {
            ...
            const resolvedFeePayer = methodDetails.feePayer === true && requestAllowsFeePayer
                ? typeof request.feePayer === 'object'
                    ? request.feePayer
                    : feePayer
                : undefined;
            ...
            switch (payload.action) {
                case 'open':
                    sessionReceipt = await handleOpen(store, client, challenge, payload, methodDetails, resolvedFeePayer, methodDetails.feePayer === true, feePayerPolicy, waitForConfirmation);
                    lastOnChainVerified.set(sessionReceipt.channelId, Date.now());
                    break;
                case 'topUp':
                    sessionReceipt = await handleTopUp(store, client, challenge, payload, methodDetails, resolvedFeePayer, methodDetails.feePayer === true, feePayerPolicy);
                    lastOnChainVerified.set(sessionReceipt.channelId, Date.now());
                    break;
                case 'voucher':
                    sessionReceipt = await handleVoucher(store, client, effectiveMinVoucherDelta, challenge, payload, methodDetails, channelStateTtl, lastOnChainVerified);
                    break;
                case 'close':
                    sessionReceipt = await handleClose(store, client, challenge, payload, methodDetails, account, resolvedFeePayer);
                    break;
                default:
                    throw new BadRequestError({
                        reason: `unknown action: ${payload.action}`,
                    });
            }
            ...
            if (envelope &&
                ...
                if (parameters.sse)
                    sessionReceipt = Transport.markPrepaidSessionTick(sessionReceipt);
            }
...
function assertSettlementSender(parameters) {
    ...
    if (!sender)
        return;
    if (sender.toLowerCase() === payee.toLowerCase())
        return;
    throw new BadRequestError({
        reason: `Cannot ${operation} channel ${channelId}: tx sender ${sender} is not the channel payee ${payee}. ` +
 ...
 */
export async function settle(store, client, channelId, options) {
    ...
    const resolvedEscrow = options?.escrowContract ??
        defaults.escrowContract[chainId];
    if (!resolvedEscrow)
        throw new Error(`No escrow contract for chainId ${chainId}.`);
    ...
    await store.updateChannel(channelId, (current) => {
        if (!current)
            return null;
        const nextSettled = settledAmount > current.settledOnChain ? settledAmount : current.settledOnChain;
 ...
 */
function validateOnChainChannel(onChain, recipient, currency, amount) {
 ...
 */
async function handleOpen(store, client, challenge, payload, methodDetails, feePayer, isSponsored, feePayerPolicy, waitForConfirmation) {
    ...
    const amount = challenge.request.amount ? BigInt(challenge.request.amount) : undefined;
    const validateOpenVoucher = async (onChain) => {
        validateOnChainChannel(onChain, recipient, currency, amount);
        const authorizedSigner = onChain.authorizedSigner === '0x0000000000000000000000000000000000000000'
            ? onChain.payer
    ...
    if (!updated)
        throw new VerificationFailedError({ reason: 'failed to create channel' });
    return createSessionReceipt({
        challengeId: challenge.id,
 ...
 */
async function handleTopUp(store, client, challenge, payload, methodDetails, feePayer, isSponsored, feePayerPolicy) {
    const channelId = ChannelStore.normalizeChannelId(payload.channelId);
    const channel = await store.getChannel(channelId);
    if (!channel) {
        throw new ChannelNotFoundError({ reason: 'channel not found' });
 ...
 */
async function handleVoucher(store, client, minVoucherDelta, challenge, payload, methodDetails, channelStateTtl, lastOnChainVerified) {
    const channelId = ChannelStore.normalizeChannelId(payload.channelId);
    const channel = await store.getChannel(channelId);
    if (!channel) {
        throw new ChannelNotFoundError({ reason: 'channel not found' });
    ...
    const onChain = await (async () => {
        if (isStale) {
            ...
            if (onChainChannel.closeRequestedAt !== 0n) {
                await store.updateChannel(channelId, (current) => current ? { ...current, closeRequestedAt: onChainChannel.closeRequestedAt } : current);
            }
 ...
 */
async function handleClose(store, client, challenge, payload, methodDetails, account, feePayer) {
    const channelId = ChannelStore.normalizeChannelId(payload.channelId);
    const channel = await store.getChannel(channelId);
    if (!channel) {
        throw new ChannelNotFoundError({ reason: 'channel not found' });
    ...
    const isUntouchedZeroClose = voucher.cumulativeAmount === 0n && channel.spent === 0n && onChain.settled === 0n;
