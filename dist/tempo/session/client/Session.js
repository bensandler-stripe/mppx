...
import { isEventStream, readSessionChallengeAmount, requireSessionCredentialContext, } from '../precompile/Protocol.js';
...
import { handleSseNeedVoucher, isTip1034SessionChallenge, postTopUp, wrapSseResponse, } from './Transports.js';
...
export function session(parameters = {}) {
    const { account, autoSwap: autoSwapParameter, channelStore, decimals = defaults.decimals, escrow: escrowOverride, getClient: getClientParameter, maxDeposit: maxDepositParameter, topUpAmount: topUpAmountParameter, onChannelUpdate, resolveAccount, } = parameters;
    ...
    const method = Method.toClient(Methods.session, {
        ...
        async createCredential({ challenge, context }) {
            ...
            const payload = await executeCredentialPlan(planCredential({
                account,
                entry,
                context,
                decimals,
                maxDeposit,
                resolved,
            }), sink, AutoSwap.resolve(context?.autoSwap ?? autoSwapParameter, AutoSwap.defaultCurrencies));
            return serializeCredential(challenge, payload, resolved.chainId, account);
        },
    });
    const topUpChannelIfNeeded = async ({ challenge, channel, deposit, fetch, input, requiredCumulative, }) => {
        ...
        const additionalDeposit = resolveAutomaticTopUp({
            ...
            maxDeposit,
            requiredCumulative,
            suggestedDeposit: challenge.request.suggestedDeposit === undefined
                ? undefined
        ...
        if (nextDeposit === channel.deposit)
            return;
    ...
    MethodChallenge.register(method, async ({ challenge, context, fetch, input }) => {
        ...
        if (requiredCumulative <= deposit && deposit === channel.deposit)
            return;
        const account = await resolveCredentialAccount(resolved, sessionContext, channel);
        if (!canSignDescriptor(account, channel.descriptor))
            return;
