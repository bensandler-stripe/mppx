...
const PaymentOffer = z.looseObject({
    amount: z.optional(z.union([z.null(), z.string().check(z.regex(/^(0|[1-9][0-9]*)$/, 'Invalid amount'))])),
 ...
 */
export const PaymentInfo = z.pipe(z
    .looseObject({
    amount: z.optional(z.union([z.null(), z.string().check(z.regex(/^(0|[1-9][0-9]*)$/, 'Invalid amount'))])),
    currency: z.optional(z.string()),
    description: z.optional(z.string()),
    ...
    .check(z.refine((value) => value.offers === undefined || Object.keys(value).every((key) => key === 'offers'), 'Cannot mix offers with flat payment info fields')), z.transform((value) => {
    if (value.offers)
    ...
    for (const [key, field] of Object.entries(value)) {
        if (key === 'offers')
            continue;
        if (paymentInfoFieldNames.has(key) || field !== undefined)
            offer[key] = field;
    }
    return { offers: [offer] };
}));
